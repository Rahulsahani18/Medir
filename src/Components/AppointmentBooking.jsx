...
const BookAppointment = () => {
  ...
  // Function to clear all localStorage data for booking
  ...
  const fetchTimeSlots = async (date) => {
    ...
    try {
      ...
      if (response.data?.status === true) {
        ...
        console.log("Time slots received:", timeSlotsData);
        setTimeSlots(timeSlotsData);
      } else {
        console.log("No time slots found or status false");
        setTimeSlots({});
  ...
  // Fetch time slots ONLY when date is selected AND date/month/year changed
  useEffect(() => {
    if (doctor?.id && selectedDate && currentStep === 2) {
      ...
      // Only fetch if date changed OR month/year changed (which would change the actual date)
      if (dateChanged || monthChanged || yearChanged) {
        console.log("Fetching time slots - Date/Month/Year changed");
  ...
  }, [doctor?.id, selectedDate, currentStep, currentMonth, currentYear]);
  ...
  // Steps - 4 steps total
  const steps = [
    { num: 1, label: "Appointment Type", active: currentStep >= 1 },
    { num: 2, label: "Date & Time", active: currentStep >= 2 },
    { num: 3, label: "Basic Information", active: currentStep >= 3 },
    { num: 4, label: "Confirmation", active: currentStep >= 4 },
  ];
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 1 && !selectedConsultationTypeId) {
    ...
    if (currentStep === 3) {
      ...
      // If we're on step 3 and clicking Next, submit the booking
  ...
  // Save state to localStorage (except time slots)
  useEffect(() => {
    // Only save if we're not on the confirmation step (step 4)
    if (currentStep !== 4) {
  ...
  const downloadReceiptPDF = async () => {
    try {
      ...
      const details = [
        ...
        { label: "Doctor:", value: doctor?.name || "Dr. Unknown" },
        { label: "Specialty:", value: doctor?.specialty || "General Medicine" },
      ...
      const fees = [
        { item: "Consultation Fee", amount: doctor?.fee || 150 },
    ...
    const printContent = `
      ...
      .padStart(4, "0")}</div>
          ...
          <div class="detail-item">
            ...
            <div class="detail-value">${doctor?.name || "Dr. Unknown"}</div>
          </div>
          <div class="detail-item">
      ...
      formData.lastName
        ...
        <table class="fees-table">
          <tr>
            ...
            <td align="right">$${(doctor?.fee || 150).toFixed(2)}</td>
          </tr>
          <tr>
            <td>Service Fee</td>
          ...
          <tr class="total-row">
            <td>Total Amount</td>
            <td align="right">$${((doctor?.fee || 150) + 15 + 12.5).toFixed(
              2
            )}</td>
          </tr>
        </table>
      ...
      <div className="bookApntmt__wrapper container">
        ...
        <div className="bookApntmt__card">
          ...
          <div className="bookApntmt__doctorHeader">
            <img
              src={doctor?.image || "https://via.placeholder.com/85"}
            ...
            <div className="bookApntmt__doctorInfo">
              <h3>
                ...
                <span className="bookApntmt__rating">
                  <Star size={12} fill="white" /> {doctor?.rating || "4.5"}
                </span>
              </h3>
                ...
                <div className="bookApntmt__calendarLeft">
                  ...
                  {/* {selectedDate && loadingTimeSlots && (
                    <div className="text-center p-3">
                      ...
                      <span className="ms-2">Loading time slots...</span>
                    </div>
                  )} */}
                </div>
            ...
            <div className="bookApntmt__confirmation">
              ...
              <div className="bookApntmt__appointmentDetails">
                ...
                <div className="bookApntmt__totalAmount">
                  ...
                  <span>${((doctor?.fee || 150) + 15 + 12.5).toFixed(2)}</span>
                </div>
              </div>
